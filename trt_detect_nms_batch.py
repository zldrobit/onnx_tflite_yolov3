    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        with engine.create_execution_context() as context:
            ...
            for isample in range(batch_size):
                # assume batch_size = 1
                ndets = num_detections[isample]
                classes_ = nmsed_classes[isample, :ndets]
                classes_ = torch.Tensor(classes_)
                probs = nmsed_scores[isample, :ndets]
                # print('probs', probs)
                boxes = nmsed_boxes[isample, :ndets, :]
                # print('boxes', boxes)
        ...
        # print("avg time:", (end - start) / 100)
                ...
                print('%02d: %sDone. (%.3fs)' % (isample, s, time.time() - t))
